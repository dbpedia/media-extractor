...
class FlickrMediaProvider(
  ...
  // Members declared in org.dbpedia.media_extractor.media_provider.MediaProvider
  def addLookupResultsToRDFGraph(lookupResults: Set[FlickrSearchResult], rdfGraph: Model): Unit = ???
